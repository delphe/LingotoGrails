    ...
    def remotePaginate = {attrs ->
        ...
        if (currentstep > firststep) {
            ...
            writer << wrapInListItem(bootstrapEnabled,remoteLink(linkTagAttrs.clone()) {
                (attrs.prev ? attrs.prev : messageSource.getMessage('paginate.prev', null, messageSource.getMessage('default.paginate.prev', null, '<<', locale), locale))
            })
        }
        ...
        if (currentstep < laststep) {
            ...
            linkParams.offset = offset + max
            writer << wrapInListItem(bootstrapEnabled,remoteLink(linkTagAttrs.clone()) {
                (attrs.next ? attrs.next : messageSource.getMessage('paginate.next', null, messageSource.getMessage('default.paginate.next', null, '>>', locale), locale))
            })
        }
